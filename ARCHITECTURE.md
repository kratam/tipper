...
- **`odds-api`** (`providers/odds-api/`): odds-api.io kliens (`/events`, `/odds/multi`, `/leagues`), státusz `pending→scheduled`/`live→live`/`settled→finished`, score csak settled-nél (`periods.ft ?? fulltime ?? top-level` — a `ft` a rendes idő / 90 perc, a top-level OT/tizenegyes-inclusive, ezért a `ft` az elsődleges; részletek lent: odds-api.io v3 → Eredmény-szemantika), bookmaker-preferencia `TippmixPRO`→`Bet365`→bármely `ML` piac. Odds tornaszinten: a pending események ID-jait **10-es batchekben** kéri le a `/odds/multi`-n (a rate limit miatt — lásd lent).
...
Base `https://api.odds-api.io/v3`, auth `?apiKey=`. World Cup slug `international-fifa-world-cup` (104 esemény) — a `league=<slug>` (egyes szám) paraméterrel; a rövidített `fifa-world-cup`/`world-cup` → `HTTP 404 "League not found"`. A `sport=football` **liga nélkül** `HTTP 500`-at ad (a 12 000+ liga túl nagy válasz), ezért a kód mindig konkrét `league`-gel kér.

**Eredmény-szemantika (`scores`/`periods`) — kieséses meccsek (verifikálva: hivatalos doc + valós VB-adat, 2026-06):** a settled esemény `scores` objektuma `{ home, away, periods }`. A `periods` kulcsai (docs.odds-api.io): `p1…pN` (félidők/harmadok), **`ft` = rendes idő / „regulation" (focinál a 90 perc)**, `ot` = hosszabbítás/extra time, `ap` = tizenegyes-párbaj — utóbbi kettő **külön** kulcsban. A top-level **`scores.home/away` az OT/tizenegyes-inclusive végeredmény** (a doc szerint; a megfigyelt hoki-válaszban ráadásul az utolsó periódus értéke jött — tehát regulation-forrásként **megbízhatatlan**). Ezért a `normalize` az **`ft`-et veszi elsődlegesen** → kieséses meccsnél a **rendes játékidő** számít: pl. 90 perc 1-1 → a döntetlen (1-1) tipp nyer, a 11-esek/hosszabbítás nem módosítanak. Valós VB-csoportadatban a settled meccsek **100%-ánál** jelen volt az `ft` (`periods = {ft, p1}`, 0 hiányzó). A `?? top-level` fallback csak `ft` hiányában lépne be — ami kieséses meccsnél hibás (tizenegyes-inclusive) eredményt pontozna; settled meccsen eddig sosem fordult elő, ezért szándékosan megmaradt.

**Instabilitás — 500-flood + retry:** az odds-api terhelés alatt sűrűn ad `HTTP 500 "Failed to fetch events"`-et **ugyanarra** a kérésre (megfigyelt: a foci-VB `/events` ~7 próbából 1-szer jött be). A `createOddsApiClient().getJson` ezért **csak az 500-ra** retry-zik exponenciális backoff-fal, max `ODDS_API_MAX_ATTEMPTS = 4` próba (1 + 3, 1s→2s→4s) — konzervatívan, hogy ne floodolja az API-t. A 4xx (404 League not found, 401 rossz kulcs) **azonnal** dob (végleges, nem átmeneti). A `skip`/`limit` lapozó paraméterek megbízhatatlanok (`skip>0` és `limit≥100` is 500-at adott), ezért nem használjuk.

**League slug átnevezés veszély:** az odds-api időnként átnevezi a league slugot (2026-06-12 incidens: `international-world-cup` → `international-fifa-world-cup`), és a régi slugra **üres listát** ad hiba helyett — a sync napokig némán no-op marad (nincs fixtures/odds frissítés, nincs pontozás). A `syncFixtures` ilyenkor `console.error`-t logol („Provider returned 0 games... possible league slug rename"), ami a Vercel logban látszik. Javítás: a `tournaments.provider_league_slug` átírása az új slugra (az event ID-k változatlanok maradnak, a következő cron-tick mindent bepótol).

Kliens: `fetchEvents(sport, leagueSlug)`, `fetchMultiEventOdds(eventIds[], bookmakers)` → `/odds/multi?eventIds=<vesszős>` (max **10** eventId/hívás, tömb válasz, elemenként a `/odds` alakjával), `fetchEventOdds(eventId, bookmakers)` (egyes esemény, fallback), `fetchLeagues(sport)`. A `bookmakers` paraméter kötelező.
...
- **Eredmény**: mindkét provider a **rendes játékidőt** pontozza, a hosszabbítás/tizenegyes nem számít — api-sports (hoki): csak a 3 period (`parseRegulationScore`, az `overtime`/`penalties` eldobva); odds-api (foci): a `periods.ft` (90 perc), lásd odds-api.io v3 → Eredmény-szemantika
