...
- **`odds-api`** (`providers/odds-api/`): odds-api.io kliens (`/events`, `/odds/multi`, `/leagues`), státusz `pending→scheduled`/`live→live`/`settled→finished`, score csak settled-nél (`periods.ft ?? fulltime ?? top-level` — a valós API a `ft` kulcsot küldi; a top-level score hosszabbítást is tartalmazhat, ezért a `ft` az elsődleges), bookmaker-preferencia `TippmixPRO`→`Bet365`→bármely `ML` piac. Odds tornaszinten: a pending események ID-jait **10-es batchekben** kéri le a `/odds/multi`-n (a rate limit miatt — lásd lent).
...
Base `https://api.odds-api.io/v3`, auth `?apiKey=`. World Cup slug `international-fifa-world-cup` (104 esemény).
